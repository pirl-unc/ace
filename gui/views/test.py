...
class BlockAssignment:
    ...
    def num_violations(self) -> float:
        ...
        for peptide_id in peptide_ids:
            peptide_pool_dict[peptide_id] = list(
                df_assignments.loc[df_assignments['peptide_id'] == peptide_id, 'pool_id'].unique())
    ...
    def is_optimal(
        ...
        for peptide_id in list(df_assignments['peptide_id'].unique()):
            ...
            if len(pool_ids) != num_coverage:
                if verbose:
                    logger.info(
                        'Assignment does not meet constraint #1: peptide %s is in %i different pools (expected: %i).' %
            ...
            if len(value) > 1:
                if verbose:
                    if constraint_2_bool:
                        logger.info(
                            "Assignment does not meet constraint #2: there are peptides that do not belong to exactly one unique combination of pool IDs.")
                    logger.info("\tPools %s have the following peptides: %s." % (key, ','.join(value)))
                constraint_2_bool = False
        if constraint_2_bool:
            if verbose:
                logger.info(
                    'Assignment meets constraint #2: each peptide belongs to exactly one unique combination of pool IDs.')
        ...
        if len(df_assignments['pool_id'].unique()) != num_pools:
            ...
            if verbose:
                logger.info(
                    'Assignment does not meet constraint #4: %i extra pool(s) than the minimum possible number of pools (%i).' %
                    (num_extra_pools, num_pools))
            constraint_4_bool = False
        if constraint_4_bool:
            if verbose:
                logger.info(
                    'Assignment meets constraint #4: there is an optimal (minimal) number of pools (%i).' % num_pools)
    ...
    def minimize_violations(
        ...
        for _ in range(0, shuffle_iters):
            ...
            if curr_num_violations < min_violations:
                if verbose:
                    logger.info(
                        '\tFound a better assignment; current number of violations: %i, new number of violations: %i' %
