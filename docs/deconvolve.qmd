...
usage: ace deconvolve [-h]
    ...
    [--background-spot-count BACKGROUND_SPOT_COUNT]
    [--verbose VERBOSE]
```
...
| --------------------------- | ------------------------------------------------------------------------ |
| `--method` | Statistical deconvolution mode (default: 'cem').           |
| `--background-spot-count` | Background spot count (default: 'auto'). If 'auto', ACE automatically estimates the background spot count. The background spot count is used as negative control. Peptides with an estimated spot count less than or equal to the background spot count will be excluded as putative immunogenic hit peptides. Automatic estimation is applicable when the deconvolution method is constrained expectation-maximization ('cem'). |
